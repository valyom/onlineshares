...
public  class BnbRates implements PokSharesAtDate {
    ...
    private boolean processDate(LocalDate date) {
        ...
        Set<String> securitiesToFind = new HashSet<>(); // security();
        List<String> securitiesFound = new ArrayList<>(); 

        String specificSectoFind =  security();
        if (specificSectoFind == null) {
            securitiesToFind.add(BnbUsdRates.SECURITY);
            securitiesToFind.add(BnbGoldRates.SECURITY);
        }
        else {
            securitiesToFind.add(specificSectoFind);
        }
        int succCnt = 0;
